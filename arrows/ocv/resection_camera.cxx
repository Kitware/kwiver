...
{
  double reproj_accuracy = 2.; // DBL_EPSILON
...
resection_camera::get_configuration() const
{
  ...
  config->set_value("reproj_accuracy", d_->reproj_accuracy, "desired re-projection positive accuracy");
...
{
  d_->reproj_accuracy = config->get_value<double>("reproj_accuracy", d_->reproj_accuracy);
  d_->max_iterations = config->get_value<int>("max_iterations", d_->max_iterations);
}
...
{
  bool good_conf = true;
  double reproj_accuracy = config->get_value<double> ("reproj_accuracy", d_->reproj_accuracy);
  if (reproj_accuracy <= 0.0)
  {
    LOG_ERROR(d_->m_logger,
      "reproj_accuracy parameter is " << reproj_accuracy << ", but needs to be positive.");
    good_conf = false;
  }
  ...
  {
    LOG_ERROR(d_->m_logger,
      "max iterations is " << max_iterations << ", needs to be greater than zero.");
    good_conf = false;
  }
  return good_conf;
}
...
) const
{
  ...
  const auto err = calibrateCamera(objPts, imgPts,
    imgSize, cv_K, dist_coeffs, vrvec, vtvec, flags,
    TermCriteria(TermCriteria::COUNT+TermCriteria::EPS, d_->max_iterations, reproj_error)
  );
  if (err>reproj_error)
  {
    LOG_WARN(d_->m_logger, "estimated re-projection error "
      << err << " > " << reproj_error << " expected re-projection error");
  }
  Mat rvec=vrvec[0];
  Mat tvec=vtvec[0];
