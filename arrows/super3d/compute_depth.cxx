...
::set_configuration( vital::config_block_sptr in_config )
{
  ...
  d_->iterations = config->get_value< unsigned int >( "iterations",
                                                      d_->iterations );
...
{
  frame =
    vil_crop( frame, roi.min_x(), roi.width(), roi.min_y(), roi.height() );
  cam = crop_camera( cam, roi.min_x(), roi.min_y() );

  return std::unique_ptr< world_space >( new world_angled_frustum( cam,
                                                                   world_plane_normal,
                                                                   d_min,
                                                                   d_max,
                                                                   roi.width(),
...
::compute( std::vector< kwiver::vital::image_container_sptr > const& frames_in,
           ...
           std::vector< kwiver::vital::image_container_sptr > const& masks_in )
const
{
  // convert frames
...
#pragma omp parallel for schedule(static, 1)
    for( int i = 0; i < static_cast< int >( masks.size() ); i++ )
    {
      ...
      auto vxl_mask = vxl::image_container::vital_to_vxl(
         masks_in[ i ]->get_image() );
      if( !vxl_mask )
      {
        continue;
      }
  ...
  std::unique_ptr< world_space > ws =
    d_->compute_world_space_roi( cameras[ ref_frame ], frames[ ref_frame ],
  ...
  else
  {
    ...
    f =
      std::bind1st( std::mem_fun(
                      &compute_depth::priv::iterative_update_callback ),
                    this->d_.get() );
