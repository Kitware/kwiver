...
void
world_space
::warp_image_to_depth( const vil_image_view< PixT >& in,
                       ...
                       PixT fill )
{
  ...
  char buf[ 60 ];
  sprintf( buf, "images/slice%2f_frame%d_%d.png", wpts[ 0 ][ 2 ], f,
           wip.interpolator_ );
...
::warp_image_to_depth( const vil_image_view< double >& in,
                       vil_image_view< double >& out,
                       const vpgl_perspective_camera< double >& cam,
                       double depth_slice, int f, double fill );

template void world_space
::warp_image_to_depth( const vil_image_view< vxl_byte >& in,
                       vil_image_view< vxl_byte >& out,
                       const vpgl_perspective_camera< double >& cam,
                       double depth_slice, int f, vxl_byte fill );

template void world_space
::warp_image_to_depth( const vil_image_view< bool >& in,
