  ...
  const std::vector< vil_image_view< bool > >& masks )
{
  ...
  std::vector< vpgl_perspective_camera< double > > warp_cams = ws->warp_cams(
    cameras, ref_frame );
  ...
  for( unsigned int k = 0; k < S; k++ )
  {
    ...
    for( unsigned int f = 0; f < frames.size(); f++ )
    {
      ...
      if( !masks.empty() )
      {
        ws->warp_image_to_depth( masks[ f ], warp_mask, warp_cams[ f ], s, f,
                                 false );
      }
...
#pragma omp parallel for
      for( int64_t j = 0; j < warp_ref.nj(); j++ )
      {
        for( unsigned int i = 0; i < warp_ref.ni(); i++ )
        {
          if( warp( i, j ) == -1 ||
              ( !masks.empty() &&
                ( warp_ref_mask( i, j ) || warp_mask( i, j ) ) ) )
          {
            continue;
          }
                        ...
                        std::vector< vpgl_perspective_camera< double > > const& cameras )
{
  ...
  for( unsigned i = 0; i < 3; ++i )
  {
    for( unsigned j = 0; j < 3; ++j )
    {
      ...
      {
        auto p1 =
          cam.project( vgl_homg_point_3d< double >( near[ 0 ], near[ 1 ],
                                                    near[ 2 ] ) );
        auto p2 =
          cam.project( vgl_homg_point_3d< double >( far[ 0 ], far[ 1 ],
                                                    far[ 2 ] ) );
