...
::set_configuration( vital::config_block_sptr config_in )
{
  ...
  this->d->m_train_directory = config->get_value< std::string >(
    "train_directory" );
                 ...
                 vital::category_hierarchy_sptr object_labels )
{
  ...
  for( unsigned fid = 0; fid < image_names.size(); ++fid )
  {
    ...
    kwiver::vital::detected_object_set_sptr detections_ptr =
      groundtruth[ fid ];
    ...
    if( m_crop_left )
    {
      original_image = cv::Mat( original_image,
                                cv::Rect( 0, 0, original_image.cols / 2,
                                          original_image.rows ) );
    }
    ...
    if( m_resize_option != "disabled" )
    {
      resized_scale = format_image( original_image, resized_image,
                                    m_resize_option, m_scale, m_resize_i,
                                    m_resize_j );
    ...
    else
    {
      ...
      if( m_resize_option == "chip_and_original" )
      {
        ...
        kwiver::vital::bounding_box_d roi_box( 0, 0,
                                               scaled_original.cols,
                                               scaled_original.rows );
  ...
  vital::category_hierarchy_sptr object_labels )
{
  ...
  for( auto detection = all_detections->cbegin(); detection != ie;
       ++detection )
  {
