...
::image_container( const std::string& filename )
{
  ...
  gdal_dataset_.reset(
      static_cast< GDALDataset* >( GDALOpen( filename.c_str(),
                                             GA_ReadOnly ) ) );
  ...
  if( osrs.GetAuthorityCode( "GEOGCS" ) )
  {
    ...
    md->add< vital::VITAL_META_CORNER_POINTS >(
      vital::geo_polygon( points,
                          atoi( osrs.GetAuthorityCode( "GEOGCS" ) ) ) );
  }
...
::get_image() const
{
  ...
  {
    GDALRasterBand* band = gdal_dataset_->GetRasterBand(
       static_cast< int >( i ) );
