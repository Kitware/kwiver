...
#define IDX_COLUMN_MAJOR( c, r, p, nrows, \
                          ncols ) ( ( c ) + ( ( r ) + ( p ) * ( ncols ) ) * \
                                    ( nrows ) )
#define IDX_OPENCV( c, r, p, nrows, ncols, \
                    nchannels ) ( ( ( ( c ) * ( ncols ) + ( r ) ) * \
                                    ( nchannels ) ) + ( p ) )
...
convert_mx_image( const kwiver::vital::image_container_sptr image )
{
  ...
  MxArray* mx_image =
    new MxArray( mxCreateNumericArray( 3, dims, mxUINT8_CLASS, mxREAL ) );
    ...
    for( size_t c = 0; c < cols; c++ )
    {
      for( size_t r = 0; r < rows; r++ )
      {
        mx_mem[ IDX_COLUMN_MAJOR( r, c, p, rows, cols ) ] =
          cv_mem[ IDX_OPENCV(  r, c, ( planes - p - 1 ), rows, cols,
                               planes ) ];
      }
    }
  }
...
convert_mx_image( const MxArraySptr mx_image )
{
...
// ------------------------------------------------------------------
MxArraySptr
create_mxByteArray( size_t r, size_t c )
{
  return std::make_shared< MxArray >( mxCreateNumericMatrix( r, c,
                                                             mxUINT8_CLASS,
                                                             mxREAL ) );
}
...
// ------------------------------------------------------------------
MxArraySptr
create_mxIntArray( size_t r, size_t c )
{
  MxArray* mxa =
    new MxArray( mxCreateNumericMatrix( r, c, mxINT32_CLASS, mxREAL ) );
  return MxArraySptr( mxa );
}

// ------------------------------------------------------------------
MxArraySptr
create_mxDoubleArray( size_t r, size_t c )
{
  MxArray* mxa =
    new MxArray( mxCreateNumericMatrix( r, c, mxDOUBLE_CLASS, mxREAL ) );
  return MxArraySptr( mxa );
}
