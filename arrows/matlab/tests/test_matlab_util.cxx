...
IMPLEMENT_TEST( image_conversion )
{
  ...
  cv::Mat ocv_image;
  ocv_image = cv::imread( test_read_file, CV_LOAD_IMAGE_COLOR );   // Read the
                                                                   // file
...
#if DEBUG
  cv::namedWindow( "input OCV image", cv::WINDOW_AUTOSIZE ); // Create a window
                                                             // for display.
  cv::imshow( "input OCV image", ocv_image ); // Show our image inside it.
  cv::waitKey( 000 ); // pause for keystroke
#endif

  kwiver::arrows::matlab::MxArraySptr mx_image =
    kwiver::arrows::matlab::convert_mx_image( ic_sptr );
...
#if DEBUG
  cv::namedWindow( "output OCV image", cv::WINDOW_AUTOSIZE ); // Create a window
                                                              // for display.
  ...
  bool isEqual = ( cv::sum( ocv_image != ocv_ic_mat ) == cv::Scalar(
    0, 0, 0,
    0 ) );
  if( !isEqual )
  {
