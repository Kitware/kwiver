...
void launch_depth_kernel( double* d_depth, double* d_weight,
                          int depthmap_dims[ 2 ],
...
::set_configuration( vital::config_block_sptr in_config )
{
  ...
  d_->ray_potential_epsilon =
    config->get_value< double >( "ray_potential_epsilon",
                                 d_->ray_potential_epsilon );
  d_->ray_potential_delta =
    config->get_value< double >( "ray_potential_delta",
                                 d_->ray_potential_delta );
  d_->voxel_spacing_factor =
    config->get_value< double >( "voxel_spacing_factor",
                                 d_->voxel_spacing_factor );
...
void
copy_camera_to_gpu( kwiver::vital::camera_perspective_sptr camera,
  ...
  vector_3d& spacing ) const
{
  ...
  CudaErrorCheck( cudaMemcpy( h_volume->data(), d_volume.get(),
                              vsize * sizeof( double ),
                              cudaMemcpyDeviceToHost ) );

  volume = std::make_shared< simple_image_container >(
    image_of< double >( h_volume,
                        reinterpret_cast< const double* >( h_volume->data() ),
                        d_->grid_dims[ 0 ], d_->grid_dims[ 1 ],
                        d_->grid_dims[ 2 ],
                        1, d_->grid_dims[ 0 ],
                        d_->grid_dims[ 0 ] * d_->grid_dims[ 1 ] ) );
}
