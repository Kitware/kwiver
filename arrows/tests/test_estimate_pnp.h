...
TEST ( estimate_pnp, ideal_points )
{
  ...
  auto est_cam = est_pnp.estimate( pts_projs, pts_3d,
                                   cam->intrinsics(), inliers );
...
TEST ( estimate_pnp, outlier_points )
{
  ...
  for( auto const& track : tracks->tracks() )
  {
    ...
    {
      auto const fts =
        std::dynamic_pointer_cast< feature_track_state >( *track->find(
                                                            frame1 ) );
      pts_projs.push_back( fts->feature->loc() );
    }
  }
  ...
  EXPECT_MATRIX_SIMILAR( cam->center(),
                         est_cam->center(), outlier_center_tolerance );
