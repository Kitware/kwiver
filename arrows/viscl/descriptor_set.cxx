...
::descriptors() const
{
  ...
  queue->enqueueReadBuffer( *data_().get(), CL_TRUE, 0, data_.mem_size(),
                            buf );
  queue->finish();
...
descriptors_to_viscl( const vital::descriptor_set& desc_set )
{
  ...
  {
    viscl::buffer buf = viscl::manager::inst()->create_buffer< cl_int4 >(
       CL_MEM_READ_WRITE, viscl_descr.size() );
    viscl::cl_queue_t queue = viscl::manager::inst()->create_queue();
    queue->enqueueWriteBuffer( *buf().get(), CL_TRUE, 0,
                               buf.mem_size(), &viscl_descr[ 0 ] );
    queue->finish();
    return buf;
  }
