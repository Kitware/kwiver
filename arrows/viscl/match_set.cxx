...
::size() const
{
  ...
  queue->enqueueReadBuffer( *data_().get(), CL_TRUE, 0,
                            data_.mem_size(), &viscl_matches[ 0 ] );
  queue->finish();
...
matches_to_viscl( const vital::match_set& m_set )
{
  ...
  viscl::buffer viscl_data = viscl::manager::inst()->create_buffer< int >(
     CL_MEM_READ_WRITE, buf.size() );
  viscl::cl_queue_t queue = viscl::manager::inst()->create_queue();
  queue->enqueueWriteBuffer(
    *viscl_data().get(), CL_TRUE, 0, viscl_data.mem_size(), &buf[ 0 ] );
  queue->finish();
