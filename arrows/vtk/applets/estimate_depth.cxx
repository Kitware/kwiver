...
#define KWIVER_CONFIG_FAIL( msg ) \
  ...
  config_valid = validate_required_input_file( "video_source", *config,
                                               main_logger ) && config_valid;

  config_valid = validate_required_input_file( "input_landmarks_file", *config,
                                               main_logger ) && config_valid;

  config_valid = validate_optional_input_file( "mask_source", *config,
                                               main_logger ) && config_valid;

  // This functions as checking for a required input directory, by not allowing
  // it to be created
  config_valid = validate_required_output_dir( "input_cameras_directory",
                                               *config, main_logger,
                                               false ) && config_valid;

  config_valid = validate_required_output_dir( "output_depths_directory",
                                               *config,
                                               main_logger ) && config_valid;
  ...
  if( !kva::compute_depth::check_nested_algo_configuration( "compute_depth",
                                                            config ) )
  {
  ...
  {
    if( !kva::video_input::check_nested_algo_configuration( "mask_reader",
                                                            config ) )
    {
...
// ----------------------------------------------------------------------------
bool
write_depth( vtkSmartPointer< vtkImageData > depth_image,
             kv::frame_id_t frame )
{
...
compute_frames_in_range()
{
  ...
  const kv::frame_id_t start_frame = config->get_value< kv::frame_id_t >(
    "batch_depth:first_frame" );
  const kv::frame_id_t end_frame = config->get_value< kv::frame_id_t >(
    "batch_depth:end_frame" );
                            ...
                            double height_max )
{
  ...
  int ref_frame = gather_depth_frames( *similar_cameras, video_reader,
                                       ...
                                       cameras_out, frames_out, masks_out,
                                       cb );
  ...
  auto crop = kwiver::arrows::core::project_3d_bounds( minpt, maxpt, *cameras_out.at(
                                                         ref_frame ),
                                                       static_cast< int >(
                                                         ref_img.width() ),
                                                       static_cast< int >(
...
run_algorithm()
{
  ...
  kwiver::arrows::core::height_range_from_3d_bounds( minpt, maxpt, height_min,
                                                     height_max );
  ...
  else
  {
    ...
    const size_t num_depth_maps = config->get_value< size_t >(
      "batch_depth:num_depth" );
...
::add_command_options()
{
  ...
  m_cmd_options->add_options()( "h,help",
    ...
    cxxopts
    ::
    value< std::string >() )( "f,frame",
                              "The frame number to compute depth for.",
                              cxxopts
                              ::
                              value< int >() )( "l,input-landmarks-file",
                                                "3D sparse features",
                                                cxxopts
                                                ::value< std::string >() )( 
    "m,mask-source",
  ...
  // positional parameters
    ( "video-source", "Video input file",
    cxxopts::value< std::string >() )( "input-cameras-dir",
  ...
  m_cmd_options->parse_positional( { "video-source", "input-cameras-dir",
                                     "output-depths-dir" } );
}
