...
#define KWIVER_CONFIG_FAIL( msg ) \
  ...
  if( !kv::algo::video_input::check_nested_algo_configuration( "video_reader",
                                                               config ) )
  {
    KWIVER_CONFIG_FAIL( "video_reader configuration check failed" );
  }
  std::string active_attribute = config->get_value< std::string >(
    "active_attribute" );
...
bool
save_mesh( vtkSmartPointer< vtkPolyData > mesh,
...
run_algorithm()
{
  ...
  if( !all_frames_ )
  {
    vtkDataArray* active = mesh->GetPointData()->GetArray(
       active_attribute_.c_str() );
    if( !active )
    {
...
::add_command_options()
{
  ...
  m_cmd_options->add_options()( "a,all-frames",
    ...
    cxxopts
    ::
    value< int >()->default_value( "-1" ) )( "h,help",
  ...
  // positional parameters
    ( "input-mesh", "Mesh input file",
    cxxopts::value< std::string >() )( "video-file", "Video input file",
                                       cxxopts::value< std::string >() )( 
    "cameras-dir",
    "Camera input directory",
    cxxopts
    ::value< std::string >() )( "output-mesh", "Mesh output file",
                                cxxopts
                                ::value< std::string >() )
  ;
