...
#define KWIVER_CONFIG_FAIL( msg ) \
  ...
  config_valid =
    validate_required_input_file( "input_landmarks_file", *config,
                                  main_logger ) &&
    config_valid;
  ...
  if( config->has_value( "output_mesh_file" ) )
  {
    ...
    if( !( extension == ".vtp" || extension == ".obj" ||
           extension == ".ply" ) )
    {
      KWIVER_CONFIG_FAIL(
        "The output_mesh_file must have a .vtp, .ply, or .obj extension" );
    }
  }
        ...
        config->get_value< std::string >( "output_volume_file" ) == "" ) )
  {
    LOG_WARN( main_logger,
              "No output files specified. Nothing will be saved." );
  }
...
::add_command_options()
{
  ...
  m_cmd_options->add_options()( "h,help",
    ...
    cxxopts
    ::
    value< std::string >() )( "l,input-landmarks-file",
                              "3D sparse features (default: "
                              +
                              d
                              ->
                              input_landmarks_file + ")",
                              cxxopts
                              ::value< std::string >() )( "m,output-mesh-file",
                                                          "Write out isocontour mesh to file (default: "
                                                          +
                                                          d
                                                          ->
                                                          output_mesh_file +
    ...
    ::
    value< std::string >() )(
    "t,isosurface-threshold",
    "isosurface extraction threshold (default: "
    +
    std
    ::
    to_string(
      d->isosurface_threshold ) + ").",
    cxxopts
    ::
    value< double >() )
  // positional parameters
    ( "input-cameras-dir", "Camera location data",
    cxxopts::value< std::string >() )( "input-depths-dir",
