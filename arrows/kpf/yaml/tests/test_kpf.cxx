...
struct user_complex_detection_t
{
  ...
  user_complex_detection_t( int d, unsigned f, const pair< double, double >& c,
    ...
    : detection_id( d ), frame_number( f ), box_corner_pt( c ),
      box_width( w ), box_height( h ), label( s ), confidence( conf ), poly_x(
        x ), poly_y( y )
  {}
};

bool
operator==( const user_complex_detection_t& lhs,
...
{
  return { { 100, 4, { 33.3, 33.3 }, 10, 20, "vehicle", 0.3, { 10, 20, 10 },
    { 10, 20, 30 } },
...
read_detections_from_stream( std::istream& is )
{
  ...
  while( reader >>
         ...
         KPF::reader< KPFC::id_t >( buffer.detection_id,
                                    KPFC::id_t::DETECTION_ID ) >>
         KPF::reader< KPFC::timestamp_t >( buffer.frame_number,
                            ...
                            const vector< user_complex_detection_t >& dets )
{
  ...
  for( const auto& det : dets )
  {
    ...
    w <<
      KPF::writer< KPFC::bbox_t >( box_adapter(
                                     det ), KPFC::bbox_t::IMAGE_COORDS ) <<
      KPF::writer< KPFC::id_t >( det.detection_id,
                                 KPFC::id_t::DETECTION_ID ) <<
      KPF::writer< KPFC::timestamp_t >( det.frame_number,
                                        KPFC::timestamp_t::FRAME_NUMBER ) <<
      KPF::writer< KPFC::poly_t >( poly_adapter(
                                     det ), KPFC::poly_t::IMAGE_COORDS ) <<
...
IMPLEMENT_TEST( kpf_packet_extraction )
{
  ...
  TEST_EQUAL( "Start of packet extraction: reader is good",
              static_cast< bool >( reader ), true );

  // read a line of packets
  reader.next();
  TEST_EQUAL( "Read one line; reader is good", static_cast< bool >( reader ),
              true );

  auto packet_buffer = reader.get_packet_buffer();
  TEST_EQUAL( "After one line: non-zero number of packets",
              packet_buffer.empty(), false );

  const auto id_header = KPF::packet_header_t( KPF::packet_style::ID,
                                               KPFC::id_t::DETECTION_ID );

  // get the detection ID
  auto id_one = reader.transfer_packet_from_buffer( id_header );
  TEST_EQUAL( "After one line: first transfer of ID succeeded", id_one.first,
              true );
  TEST_EQUAL( "After one line: reader still good",
              static_cast< bool >( reader ), true );

  // get the detection ID again (should fail but leave reader good)
  auto id_two = reader.transfer_packet_from_buffer( id_header );
  TEST_EQUAL( "After one line: second transfer of ID failed", id_two.first,
              false );
  TEST_EQUAL( "After one line: reader still good",
              static_cast< bool >( reader ), true );
