...
::set_configuration( vital::config_block_sptr in_config )
{
  ...
  size_t n = 1;
  vital::config_block_sptr source_config =
    config->subblock( source_name( n ) );
  ...
  while( source_config->available_values().size() > 0 )
  {
    ...
    auto& caps =
      d->d_video_sources[ n - 1 ]->get_implementation_capabilities();

    has_eov = has_eov && caps.capability( vi::HAS_EOV );
    has_frame_numbers = has_frame_numbers && caps.capability(
    ...
    has_abs_fr_time = has_abs_fr_time && caps.capability(
...
::open( std::string list_name )
{
  ...
  if( !ifs )
  {
    VITAL_THROW( kwiver::vital::invalid_file, list_name,
                 "Could not open file" );
  }
         ...
         vs_iter != d->d_video_sources.end() )
  {
    ...
    {
      filepath = kwiversys::SystemTools::FindFile( filepath, d->d_search_path,
                                                   true );
    }
