               ...
               matches_vec& matches )
{
  ...
  for( auto cur_feat : va )
  {
    ...
    {
      int dist =
        static_cast< int >( desc_dist( cur_feat->descriptor,
  ...
  std::vector< frame_id_t > const& putative_matches )
{
  ...
  for( auto fn_match : putative_matches )
  {
    ...
    std::set_intersection( cur_frame_track_ids.begin(),
                           cur_frame_track_ids.end(),
                           match_frame_track_ids.begin(),
                           match_frame_track_ids.end(),
                           std::inserter( tracks_in_common,
                                          tracks_in_common.begin() ) );

    std::set_union( cur_frame_track_ids.begin(), cur_frame_track_ids.end(),
                    match_frame_track_ids.begin(), match_frame_track_ids.end(),
                    std::inserter( union_of_tracks,
                                   union_of_tracks.begin() ) );
    ...
    if( m_f_estimator )
    {
      ...
      unsigned num_inliers =
        static_cast< unsigned >( std::count( inliers.begin(), inliers.end(),
                                             true ) );
  ...
  LOG_DEBUG( m_logger,
             "Of " << putative_matches.size() << " putative matches " <<
  ...
  std::vector< frame_id_t > const& putative_matches )
{
  ...
  feature_info_sptr fi1 =
    feat_tracks->frame_feature_info( frame_number, true );
  feat1 = fi1->features;
                            ...
                            feature_info_sptr fi2 )
{
  ...
  // cost.
  // Adjusting to the median cost accounts for changes in zoom.  If most
  // matches have a
  // scale change, then that should be the low cost option to pick when
  // sorting.
  for( auto& m : mwc_vec )
  {
    m.cost /= median_cost;
          ...
          kwiver::vital::frame_id_t frame_number )
{
  ...
  auto fd = std::dynamic_pointer_cast< feature_track_set_frame_data >( feat_tracks->frame_data(
                                                                         frame_number ) );
  ...
  return verify_and_add_image_matches_node_id_guided( feat_tracks,
                                                      frame_number,
                                                      putative_matching_images );
}
