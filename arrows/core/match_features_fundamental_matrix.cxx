...
::get_configuration() const
{
  ...
  config->set_value( "min_required_inlier_count",
  ...
  config->set_value( "min_required_inlier_percent",
...
::set_configuration( vital::config_block_sptr in_config )
{
  ...
  vital::algo::match_features::set_nested_algo_configuration(
    "feature_matcher", config, d_->matcher_ );

  // Other parameters
  d_->inlier_scale = config->get_value< double >( "inlier_scale" );
  d_->min_required_inlier_count = config->get_value< int >(
    "min_required_inlier_count" );
  d_->min_required_inlier_percent = config->get_value< double >(
    "min_required_inlier_percent" );
         ...
         feature_set_sptr feat2, descriptor_set_sptr desc2 ) const
{
  ...
  match_set_sptr init_matches =
    d_->matcher_->match( feat1, desc1, feat2, desc2 );

  // estimate a fundamental_matrix from the initial matches
  std::vector< bool > inliers;
  fundamental_matrix_sptr F = d_->f_estimator_->estimate( feat1, feat2,
                                                          init_matches,
                                                          inliers,
                                                          d_->inlier_scale );
  int inlier_count =
    static_cast< int >( std::count( inliers.begin(), inliers.end(),
                                    true ) );
  LOG_INFO( logger(),
