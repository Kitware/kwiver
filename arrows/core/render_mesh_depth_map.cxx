...
render_mesh_depth_map( vital::mesh_sptr mesh,
                       vital::camera_perspective_sptr camera )
{
  ...
  if( mesh->faces().regularity() == 3 )
  {
    auto const& triangles =
      static_cast< const vital::mesh_regular_face_array< 3 >& >( mesh->
                                                                 faces() );
    ...
    transform_image( zbuffer, [ ]( double d ) {
  ...
  else
  {
    LOG_ERROR( vital::get_logger(
                 "arrows.core.render_mesh_depth_map" ),
...
render_mesh_height_map( vital::mesh_sptr mesh, vital::camera_sptr camera )
{
  vital::image_of< double > height_map(
    camera->image_width(), camera->image_height(), 1 );
  // fill each pixel with infinity
  transform_image( height_map, [ ]( double ) {
                     return std::numeric_limits< double >::infinity();
                   } );
  ...
  if( mesh->faces().regularity() == 3 )
  {
    ...
    {
      vital::image_container_sptr depth_map_container = render_mesh_depth_map(
        mesh, perspective_camera );
    ...
    else
    {
      ...
      transform_image( height_map, [ ]( double h ) {
...
/// perspective camera
void
depth_map_to_height_map( vital::camera_perspective_sptr const& camera,
...
void
render_triangle( const vital::vector_2d& v1, const vital::vector_2d& v2,
                 ...
                 vital::image_of< double >& depth_img )
{
  ...
  auto Vd =
    triangle_attribute_vector( v1, v2, v3, depth_v1, depth_v2, depth_v3 );
  ...
  for( tsi.reset(); tsi.next();)
  {
    ...
    int max_x = std::min(
       static_cast< int >( depth_img.width() ) - 1, tsi.end_x() );
