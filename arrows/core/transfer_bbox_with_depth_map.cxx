...
{
  vector_3d world_pos_bottom = backproject_to_depth_map( camera, depth_map,
                                                         img_pt_bottom );
  ...
  vital::bounding_box< double > const bbox )
{
  ...
  auto bbox_bottom_center = vector_2d( ( bbox_max_x + bbox_min_x ) / 2,
                                       bbox_max_y );
  auto bbox_top_center =
    vector_2d( ( bbox_max_x + bbox_min_x ) / 2, bbox_min_y );

  vector_3d world_pos_bottom;
  vector_3d world_pos_top;

  std::tie( world_pos_bottom, world_pos_top ) =
    backproject_wrt_height( src_camera, depth_map, bbox_bottom_center,
                            bbox_top_center );
  ...
  return vital::bounding_box< double >( dest_bbox_min_x, dest_bbox_min_y,
...
// ---------------------------------------------------------------------------
transfer_bbox_with_depth_map
::transfer_bbox_with_depth_map( kwiver::vital::camera_perspective_sptr src_cam,
  ...
  for( auto det : *input_set )
  {
    ...
    try
    {
      vital::bounding_box< double > new_out_bbox =
        transfer_bbox_with_depth_map_stationary_camera( src_camera,
                                                        dest_camera, depth_map,
                                                        out_bbox );
