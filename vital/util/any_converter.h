  ...
  : public convert_base< DEST >
{
  ...
  virtual bool
  can_convert( std::any const& data ) const override
  {
    return data.type() == typeid( SRC );
  }

  virtual DEST
  convert( std::any const& data ) const override
  {
...
// ==================================================================
/// Specialization of ``std::bad_any_cast`` with a more informative "what".
...
public:
                ...
                std::string const& to_type )
  {
    ...
    {
      m_message =
        "vital::bad_any_cast: failed conversion using kwiver::vital::any_cast "
        "from type \"" + demangle( from_type ) + "\" to type \"" +
        demangle( to_type ) + "\"";
    }
    else
    {
...
public:
  ...
  /**
   * @brief Apply conversion to an std::any object.
   *
   ...
   *
   * @throws std::bad_any_cast if the conversion is not successful.
   */
  T
  convert( std::any const& data ) const
  {
    ...
    // Throw exception
    throw kwiver::vital::bad_any_cast( data.type().name(),
                                       typeid( T ).name() );
  }

  /**
   * @brief Test to see if conversion can be done.
   *
  ...
  /**
   * @brief Add converter based on convert-from type.
   *
