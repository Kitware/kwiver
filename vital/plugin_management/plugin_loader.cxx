...
::plugin_loader( std::string const& init_function,
  ...
  : m_logger( kwiver::vital::get_logger( "vital.plugin_loader" ) ),
    m_impl( new plugin_loader_impl( this, init_function,
                                    shared_lib_suffix ) )
{}
...
{
  m_impl->m_search_paths.insert( m_impl->m_search_paths.end(),
                                 path.begin(), path.end() );
...
::add_factory( plugin_factory* fact )
{
  ...
  fact->add_attribute( plugin_factory::PLUGIN_FILE_NAME,
  ...
  LOG_TRACE( m_logger,
             "Adding plugin to create interface: \"" <<
             demangle( interface_type ) << "\" from derived type: \"" <<
             demangle( concrete_type ) << "\" from file: " <<
             m_impl->m_current_filename );
...
// {
//  m_impl->m_module_map.insert( std::pair< std::string, std::string >(name,
// m_impl->m_current_filename ) );
// }
//
//// --------------------------------------------------------------------------
...
::load_from_module( path_t const& path )
{
  ...
  if( !lib_handle )
  {
    LOG_WARN( m_parent->m_logger,
              "plugin_loader::Unable to load shared library \"" <<
              path << "\" : " << DL::LastError() );
    return;
  }
  ...
  if( fp == nullptr )
  {
    ...
    LOG_WARN( m_parent->m_logger,
              "plugin_loader:: Unable to bind to function \"" <<
              m_init_function << "()\" : " << str );
