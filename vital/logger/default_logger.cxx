...
public:
  /// CTOR
  default_logger( logger_ns::logger_factory_default* p,
  ...
  {
    return m_logLevel <= kwiver_logger::LEVEL_FATAL;
  }

  virtual bool
  is_error_enabled() const
  {
    return m_logLevel <= kwiver_logger::LEVEL_ERROR;
  }

  virtual bool
  is_warn_enabled() const
  {
    return m_logLevel <= kwiver_logger::LEVEL_WARN;
  }

  virtual bool
  is_info_enabled() const
  {
    return m_logLevel <= kwiver_logger::LEVEL_INFO;
  }

  virtual bool
  is_debug_enabled() const
  {
    return m_logLevel <= kwiver_logger::LEVEL_DEBUG;
  }

  virtual bool
  is_trace_enabled() const
  {
    return m_logLevel <= kwiver_logger::LEVEL_TRACE;
  }
...
private:
  ...
  // ------------------------------------------------------------------
  void
  log_message_i(  log_level_t level,
                  std::string const&  msg,
                  std::string const& location )
  {
    ...
    {
      std::lock_guard< std::mutex > guard( lock ); // serialize access to
                                                   // stream
